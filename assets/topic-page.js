(function () {
  ...
  if (!id || !data || !root || !data.topics[id]) {
    if (root) root.innerHTML = '<p style="padding:2rem;text-align:center;color:#94a3b8">Data not available.</p>';
    return;
  }
  ...
  function migrateSolved() {
    ...
    if (set.size) localStorage.setItem(STORAGE_SOLVED, JSON.stringify(Array.from(set)));
    return set;
  }
  ...
  function getStreakGlobal() {
    try {
      return JSON.parse(localStorage.getItem(STORAGE_STREAK) || '{"lastCountedDay":"","value":0}');
    } catch (e) {
  ...
  var easyC = problems.filter(function (p) { return p.difficulty === 'easy'; }).length;
  var medC = problems.filter(function (p) { return p.difficulty === 'medium'; }).length;
  var hardC = problems.filter(function (p) { return p.difficulty === 'hard'; }).length;
  ...
  function buildTableRows() {
    return problems
      .map(function (p, i) {
        ...
        var link =
          url === '#'
            ? '<span class="prob-link" style="opacity:.75">' + escapeHtml(p.name) + '</span>'
            : '<a class="prob-link" href="' + escapeHtml(url) + '" target="_blank" rel="noopener">' + escapeHtml(p.name) + '</a>';
        return (
          '<tr data-diff="' +
          escapeHtml(diff) +
          '" data-pid="' +
  ...
  html +=
    '</div><div class="footer"><span class="footer-sign">— Mahmoud Abdullah</span></div>';
  ...
  function loadConfetti(cb) {
    ...
    if (confettiLoaded) {
      setTimeout(function () { if (window.confetti) cb(); }, 100);
      return;
    }
    confettiLoaded = true;
